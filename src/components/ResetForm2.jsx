...
import { checkResetOtp } from '../reducers/authReducer.js'

const ResetForm2 = ({ setIsOtp, setOtp, email, otp }) => {
  const {
    register,
    handleSubmit,
    getValues,
  ...
  const onSubmit = async () => {
    const response = await dispatch(
      checkResetOtp({ email, otp: Object.values(getValues()).join('') })
    )
    setOtp(Object.values(getValues()).join(''))
    setIsOtp(response.payload.success)
  }
